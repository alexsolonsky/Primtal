...
export async function POST(request:Request){
 ...
 try{const {mode}=z.object({mode:z.enum(['verify','activate','tick','capabilities','instant','free-model','demo-day','calendar-review','status'])}).strict().parse(await request.json());
 ...
 if(mode==='tick')return Response.json(await runOnce());
 // Owner operations use the existing verified web-to-DM binding. No caller-supplied user ID.
 const row=await connection();const id=row?await bound(row.owner):null;if(!id)throw new Error('Pair the project owner first.');
 ...
 if(mode==='status')return Response.json({name:p.name,calendarSelected:!!p.calendarId,consent:p.consent,stage:s?.stage,answered:s?.index,hasCard:!!s?.outbox?.pollId,model:c.model});
 await ownCalendar(c);p.calendarId=c.calendarId;await savePerson(id,p);
 if(mode==='demo-day')return Response.json(await seedDemoDay(id,c));
 if(!s||s.index!==6||s.safety&&s.safety!=='clear')throw new Error('A completed clear check-in is required.');await accept(c,s,'CALENDAR');await flush(c,s);return Response.json({stage:s.stage,calendarReady:!s.error,proposal:s.proposal});});
