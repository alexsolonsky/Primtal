...
export async function POST(request:Request){
 ...
 try{const {mode}=z.object({mode:z.enum(['verify','activate','tick','listen','capabilities','instant','free-model','demo-day','demo-day-retry','calendar-review','status'])}).strict().parse(await request.json());
 ...
 if(mode==='status'){const health=await db().prepare("SELECT id,value FROM settings WHERE id IN ('listener-heartbeat','listener-continuation')").all<{id:string;value:string}>();return Response.json({listener:Object.fromEntries(health.results.map(r=>[r.id,JSON.parse(r.value)])),name:p.name,calendarSelected:!!p.calendarId,consent:p.consent,stage:s?.stage,answered:s?.index,hasCard:!!s?.outbox?.pollId,model:c.model});}
