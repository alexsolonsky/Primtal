...
export const db=()=>{if(!env.DB) throw new Error('Storage unavailable');return env.DB;};
export function runtime(name:string){return (env as unknown as Record<string,string>)[name]||'';}
export async function seal(value:unknown){const raw=runtime('PRIMTAL_ENCRYPTION_KEY');if(!raw)throw new Error('Encryption is not configured');const key=await crypto.subtle.importKey('raw',Uint8Array.from(atob(raw),c=>c.charCodeAt(0)),'AES-GCM',false,['encrypt']);const iv=crypto.getRandomValues(new Uint8Array(12));const data=await crypto.subtle.encrypt({name:'AES-GCM',iv},key,new TextEncoder().encode(JSON.stringify(value)));return btoa(String.fromCharCode(...iv,...new Uint8Array(data)));}
export async function unseal<T>(value:string):Promise<T>{const bytes=Uint8Array.from(atob(value),c=>c.charCodeAt(0));const key=await crypto.subtle.importKey('raw',Uint8Array.from(atob(runtime('PRIMTAL_ENCRYPTION_KEY')),c=>c.charCodeAt(0)),'AES-GCM',false,['decrypt']);return JSON.parse(new TextDecoder().decode(await crypto.subtle.decrypt({name:'AES-GCM',iv:bytes.slice(0,12)},key,bytes.slice(12))));}
export type Config={botKey:string;openrouterKey:string;model:string;botId:string;workspaceId:string;userId:string;channelId:string;calendarId:string;timezone:string;hour:string;enabled:boolean;consent:boolean;automationId?:string;lastDay?:string;remoteHash?:string;support:string;name?:string};
