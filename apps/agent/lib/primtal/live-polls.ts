...
async function listen(origin:string){
 ...
 try{while(Date.now()-started<18000){
  ...
  await Promise.allSettled(cards.slice(0,5).map(async({id,s})=>{const c=await personConfig(id);if(!c.consent)return;const vote=await selectedChoice(c,s!.outbox);if(vote)await runParticipant(id);}));rounds++;await new Promise(resolve=>setTimeout(resolve,900));
